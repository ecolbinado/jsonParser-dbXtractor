...
if __name__ == '__main__':
    ...
    for amovie in jsonData['results']:
        ...
        if newmovie.check_movie_exists(conn, cursor):
            print(str(newmovie) + " already exists in our database. Updating release date instead.")
            newmovie.update_release_date(conn, cursor)
        else:
            newmovie.insert_movie(conn, cursor)

    print("'coming_soon.json' has been parsed to 'movies.db'")
